...
var loadGtfs = function (extractedFolder, db, isSpatial, callback) {
  ...
  var loadAllFiles = function (loadingCallback) {
    ...
    if (dialect === 'sqlite') {
      ...
      async.series(
        fileLoaders.map(loader => seriesCallback => loader(extractedFolder, db, seriesCallback)),
        loadingCallback
      )
    } else {
      async.auto(
        {
          loadAgency: cb => {
            loadAgency(extractedFolder, db, cb)
          },
          loadCalendar: cb => {
            loadCalendar(extractedFolder, db, cb)
          },
          loadCalendarDates: ['loadCalendar', (results, cb) => {
            loadCalendarDates(extractedFolder, db, cb)
          }],
          loadFareAttributes: cb => {
            loadFareAttributes(extractedFolder, db, cb)
          },
          loadFareRules: ['loadFareAttributes', (results, cb) => {
            loadFareRules(extractedFolder, db, cb)
          }],
          loadFeedInfo: cb => {
            loadFeedInfo(extractedFolder, db, cb)
          },
          loadFrequencies: ['loadTrips', (results, cb) => {
            loadFrequencies(extractedFolder, db, cb)
          }],
          loadRoutes: ['loadAgency', (results, cb) => {
            loadRoutes(extractedFolder, db, cb)
          }],
          loadShapes: cb => {
            loadShapes(extractedFolder, db, cb)
          },
          loadStops: cb => {
            loadStops(extractedFolder, db, cb)
          },
          loadStopTimes: ['loadStops', 'loadTrips', (results, cb) => {
            loadStopTimes(extractedFolder, db, cb)
          }],
          loadTransfers: ['loadStops', (results, cb) => {
            loadTransfers(extractedFolder, db, cb)
          }],
          loadTrips: ['loadRoutes', 'loadCalendarDates', (results, cb) => {
            loadTrips(extractedFolder, db, cb)
          }]
        },
        loadingCallback
      )
