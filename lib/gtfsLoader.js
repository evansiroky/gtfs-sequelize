...
var insertCSVInTable = function(fileIsRequired, filename, model, callback, transformer, useCargoInserter) {
  ...
  var processTable = function() {
    model.sync({force: true}).then(function() {
      ...
      if(useCargoInserter) {
        ...
        inserterConfig = util.makeInserterConfig(model)
      }
  ...
  fs.stat(filename, 
    function(err, stats) {
      if(!err || err.code != 'ENOENT') {
        ...
        if(fileIsRequired) {
          var err = new Error(filename + ' <--- FILE NOT FOUND.  THIS FILE IS REQUIRED.  THIS FEED IS INVALID.');
          console.log(err);
          callback(err);
        } else {
...
var loadAgency = function(extractedFolder, db, callback) {
  insertCSVInTable(true,
    path.join(extractedFolder, 'agency.txt'),
    db.agency,
    callback,
    function(line) {
...
var loadStops = function(extractedFolder, db, callback) {
  insertCSVInTable(true,
    path.join(extractedFolder, 'stops.txt'),
    db.stop,
    callback);
};
...
var loadRoutes = function(extractedFolder, db, callback) {
  insertCSVInTable(true,
    path.join(extractedFolder, 'routes.txt'),
    db.route,
    callback,
    function(line) {
      if(numAgencies == 1) {
...
var loadTrips = function(extractedFolder, db, callback) {
  insertCSVInTable(true,
    path.join(extractedFolder, 'trips.txt'),
    db.trip,
    callback);
};
...
var loadStopTimes = function(extractedFolder, db, callback) {
  insertCSVInTable(true,
    path.join(extractedFolder, 'stop_times.txt'),
    db.stop_time,
    callback,
    function(line) {
...
var loadFareAttributes = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'fare_attributes.txt'),
    db.fare_attribute,
    callback);
};
...
var loadFareRules = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'fare_rules.txt'),
    db.fare_rule,
    callback,
    false,
    true);
...
var loadShapes = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'shapes.txt'),
    db.shape,
    callback);
};
...
var loadFrequencies = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'frequncies.txt'),
    db.frequncy,
    callback);
};
...
var loadTransfers = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'transfers.txt'),
    db.transfer,
    callback);
};
...
var loadFeedInfo = function(extractedFolder, db, callback) {
  insertCSVInTable(false,
    path.join(extractedFolder, 'feed_info.txt'),
    db.feed_info,
    callback);
};

module.exports = function(downloadsDir, gtfsFileOrFolder, db, isPostGIS, callback) {
  ...
  fs.lstat(gtfsPath, function(err, stats) {
    ...
    if(stats.isDirectory()) {
      loadGtfs(gtfsPath, db, isPostGIS, callback);
    } else {
